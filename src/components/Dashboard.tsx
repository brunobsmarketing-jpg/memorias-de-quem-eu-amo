...
interface DashboardProps {
  ...
  onSelectBook: (book: MemoryBookJob) => void;
  onLogout?: () => void;
}
...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const [activeSection, setActiveSection] = useState<'videos' | 'books' | 'media' | 'steps'>('videos');
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      {/* SECTION 2: BOOKS GALLERY */}
      {activeSection === 'books' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
              <BookImage className="w-5 h-5 text-amber-400" /> Seus Livros de Memórias
            </h3>
            <span className="text-xs font-semibold text-slate-400">Total: {books.length} livro(s)</span>
          </div>

          {books.length === 0 ? (
            <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-12 text-center space-y-4">
              <div className="w-16 h-16 bg-slate-800 rounded-full flex items-center justify-center text-slate-500 mx-auto">
                <BookImage className="w-8 h-8 text-amber-400" />
              </div>
              <div className="max-w-md mx-auto space-y-1">
                <h4 className="text-slate-200 font-bold text-lg">Nenhum livro criado ainda</h4>
                <p className="text-slate-400 text-sm">
                  Clique no botão abaixo para montar seu primeiro Livro de Memórias.
                </p>
              </div>
              <button
                onClick={onStartCreation}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {books.map((bk) => {
                const coverPhoto = bk.pages?.[0]?.renderedImageUrl || bk.photos?.[0]?.url || 'https://images.unsplash.com/photo-1511895426328-dc8714191300?auto=format&fit=crop&w=600&q=80';
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-5 space-y-3 flex-1 flex flex-col justify-between">
                      <div>
                        <h4 className="text-lg font-bold text-slate-100 group-hover:text-amber-400 transition-colors">
                          Livro para {bk.fatherName}
                        </h4>
                        <p className="text-xs text-slate-400 mt-1">{bk.pages.length} página(s) montada(s)</p>
                      </div>

                      <div className="flex items-center justify-between text-xs text-slate-500 pt-2 border-t border-slate-800">
                        <span className="flex items-center gap-1">
                          <Calendar className="w-3.5 h-3.5" /> {new Date(bk.createdAt).toLocaleDateString('pt-BR')}
                        </span>
                        <span>{bk.photos.length} foto(s)</span>
                      </div>
